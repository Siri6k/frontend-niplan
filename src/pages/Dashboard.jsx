...
const Dashboard = () => {
  ...
  if (!authReady || isLoading) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center">
        ...
        <p className="mt-4 text-slate-500 text-xs font-bold uppercase tracking-widest animate-pulse">
  ...
  return (
    <div className="relative min-h-screen bg-slate-950 flex flex-col font-sans selection:bg-green-500/30 overflow-x-hidden pt-4 sm:pt-14 pb-14 sm:pb-24">
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto px-4 space-y-6">
        ...
        <motion.div 
          ...
          className="glass-card rounded-[2rem] p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-center gap-6 border border-white/5"
        >
          <div className="flex items-center gap-5 w-full">
            <Link to={`/b/${business?.slug}`} className="relative group shrink-0">
              ...
              <img
                ...
                className="relative w-24 h-24 sm:w-24 sm:h-24 rounded-full object-cover border-2 border-white/10 group-hover:scale-105 transition-transform duration-300"
                alt="Logo"
              />
              {isPhoneVerified === "true" && (
                <div className="absolute bottom-1 right-1 bg-green-500 p-1.5 rounded-full border-4 border-[#0f172a] shadow-lg">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                {role === "superadmin" && (
                  <Link to="/admin-dashboard" className="shrink-0 p-1.5 bg-green-500/10 text-green-500 rounded-lg border border-green-500/20 hover:bg-green-500/20 transition-all">
              ...
              <div className="flex flex-wrap gap-2 items-center">
                {role === "vendor" && isPhoneVerified !== "true" ? (
                  <button 
                    ...
                    className="flex items-center gap-2 px-3 py-1 bg-red-500/10 text-red-400 rounded-full text-[10px] font-black uppercase tracking-widest border border-red-500/20 hover:bg-red-500/20 transition-all"
                  >
                ...
                <div className="px-3 py-1 bg-white/5 text-slate-400 rounded-full text-[10px] font-black uppercase tracking-widest border border-white/5">
          ...
          <div className="flex sm:flex-col gap-3 w-full sm:w-auto shrink-0">
             <button
                ...
                className="flex-1 sm:flex-none p-4 sm:p-3 bg-white/5 hover:bg-red-500/10 rounded-2xl text-slate-400 hover:text-red-400 transition-all border border-white/5 active:scale-95 group"
                title="Déconnexion"
              >
                <LogOut size={20} className="mx-auto group-hover:translate-x-1 transition-transform" />
              </button>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <motion.div 
            ...
            className="glass-card rounded-3xl p-6 border border-white/5 flex items-center justify-between"
          >
            <div>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Total Produits</p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4">
          <div className="flex items-center gap-3 self-start">
             <div className="p-2 bg-green-500/10 text-green-500 rounded-xl border border-green-500/20">
                <LayoutGrid size={20} />
             </div>
             <h1 className="text-xl font-black text-white tracking-tight">Catalogue</h1>
          </div>
          
          <div className="flex gap-3 w-full sm:w-auto">
             <div className="relative flex-1 group sm:w-64">
                <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600 transition-colors group-focus-within:text-green-500" />
                <input 
                   type="text" 
                   placeholder="Filtrer vos articles..." 
                   className="w-full pl-11 pr-4 py-3 bg-white/5 border border-white/5 rounded-2xl outline-none focus:bg-white/[0.08] focus:border-green-500/30 text-white text-sm placeholder:text-slate-600 transition-all font-medium"
                />
             </div>
             <button
               onClick={() => {
        ...
        >
          {products.length > 0 ? (
            ...
            <div className="p-12 glass-card rounded-[2rem] border border-white/5 text-center">
              ...
              <p className="text-slate-600 text-sm mt-1">
                Commencez à ajouter vos premiers articles pour attirer des clients.
              </p>
              <button 
                onClick={() => setShowForm(true)}
                className="mt-6 text-green-500 text-xs font-black uppercase tracking-[0.2em] hover:text-green-400 transition-colors"
              >
                + Ajouter un produit
              </button>
            </div>
