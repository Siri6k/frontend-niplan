...
const AnalyticsPreview = () => {
  return (
    <section className="py-24 bg-slate-950 px-4 rounded-[3rem] lg:rounded-[5rem] overflow-hidden relative border-t border-white/5 mx-2 lg:mx-8 my-10">
      {/* Background Glow */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
        ...
        >
          ...
          <h2 className="text-3xl md:text-5xl font-black text-white leading-tight mb-8">
            Pilotez votre <span className="text-blue-400">Croissance</span> en temps réel.
          </h2>
          <p className="text-slate-400 text-lg mb-10 max-w-md">
            Ne vendez plus en aveugle. Suivez chaque vue, chaque commande et chaque clic pour comprendre ce que vos clients adorent vraiment.
          </p>

          <div className="grid grid-cols-2 gap-6">
            <div className="glass-card p-6 rounded-[2rem]">
               <h4 className="text-slate-500 text-xs font-black uppercase tracking-widest mb-2">Visibilité</h4>
            ...
            <div className="glass-card p-6 rounded-[2rem]">
               <h4 className="text-slate-500 text-xs font-black uppercase tracking-widest mb-2">Commandes</h4>
        ...
        >
          ...
          <div className="glass-card w-full max-w-md h-[400px] rounded-[2.5rem] p-8 relative overflow-hidden group">
            {/* Fake Chart Lines */}
            ...
            <div className="space-y-4">
               {[
                   { name: "Sacs à main Luxe", stats: "110 clics", color: "bg-blue-400" },
                   { name: "Montre Connectée", stats: "85 clics", color: "bg-green-400" },
                   { name: "Chaussures Sport", stats: "72 clics", color: "bg-purple-400" },
               ].map((item, i) => (
                   <div key={i} className="flex items-center gap-4 p-3 bg-white/5 rounded-xl border border-white/5">
                       <div className={`w-8 h-8 rounded-full ${item.color}/20 flex items-center justify-center`}>
                           <ShoppingBag size={14} className={item.color.replace('bg-', 'text-')} />
                       </div>
                       ...
                       <span className="text-[10px] font-black text-slate-500">{item.stats}</span>
                   </div>
               ))}
            </div>
            ...
            <motion.div 
              ...
              className="absolute top-8 right-[-10px] bg-green-500 p-3 rounded-2xl shadow-xl flex items-center gap-2 border-2 border-slate-900"
            >
              <Send size={14} className="text-white" />
              <p className="text-[10px] font-black text-white">Nouvelle Commande WhatsApp !</p>
            </motion.div>
          </div>
        </motion.div>
      </div>
